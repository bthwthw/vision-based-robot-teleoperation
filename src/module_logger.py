...
class DataLogger:
    FIELDNAMES = [
        ...
        "raw_qw", "raw_qx", "raw_qy", "raw_qz", "filt_qw", "filt_qx", "filt_qy", "filt_qz",
    ...
    def __init__(self, filename, out_dir="logs"):
        ...
        print(f"[LOGGER INFO] Ghi log vao file: {self.filepath}")
    ...
    def log(self, frame_timestamp_s, raw_pos=None, filt_pos=None,
        ...
        row["raw_gripper_dist_mm"] = raw_gripper_dist
        row["filt_gripper_dist_mm"] = filt_gripper_dist
        ...
        if q_tgt is not None and len(q_tgt) >= 6:
            for i in range(6): row[f"q_tgt_{i+1}"] = q_tgt[i]
        if pb_q is not None and len(pb_q) >= 6:
            for i in range(6): row[f"pb_q_{i+1}"] = pb_q[i]
